...
console.log(`${colors.cyan}🧪 AGI Code CLI Comprehensive Health Check${colors.reset}\n`);
...
if (packageJson) {
  ...
  // Check AGI Code configuration (stored under legacy "erosolar" key)
  if (packageJson.erosolar?.rulebookSchema) {
    logSuccess('AGI Code rulebook schema configured');
  } else {
    logWarning('AGI Code rulebook schema not configured in package.json');
    hasWarnings = true;
  }
}
...
const agentFiles = [
  { path: 'agents/agi-code.rules.json', name: 'AGI Code rulebook' },
