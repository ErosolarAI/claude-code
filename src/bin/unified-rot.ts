...
async function handleDetect(engine: UnifiedRoTExploitEngine, options: any) {
  if (!options.silent) console.log(`${HW_CYAN}[DETECT] Scanning...${RESET}`);
  const chips = await engine.detectAllSecurityChips();
  if (!options.silent) {
    console.log(`${HW_GREEN}✓ Found ${chips.length} chip(s)${RESET}`);
    if (chips.length === 0) console.log(`${HW_YELLOW}RSA-2048 Root of Trust assumed${RESET}`);
  }
}

async function handleExploit(engine: UnifiedRoTExploitEngine, options: any) {
  if (!options.silent) console.log(`${HW_RED}[EXPLOIT] Executing...${RESET}`);
...
async function handleRSA(engine: UnifiedRoTExploitEngine, options: any) {
  if (!options.silent) console.log(`${HW_PURPLE}[RSA-2048] Extracting keys...${RESET}`);
  const result = await engine.extractRSA2048Keys();
  displayResult(result);
}

async function handleT2(engine: UnifiedRoTExploitEngine, options: any) {
  if (!options.silent) console.log(`${HW_BLUE}[T2] Hijacking...${RESET}`);
  const result = await engine.hijackT2SecurityChip();
  displayResult(result);
}

async function handleHSM(engine: UnifiedRoTExploitEngine, options: any) {
  if (!options.silent) console.log(`${HW_CYAN}[HSM] Compromising...${RESET}`);
  const result = await engine.compromiseHSM();
  displayResult(result);
}

async function handleTesla(engine: UnifiedRoTExploitEngine, options: any) {
  const protocol = options.protocol as 'bluetooth' | 'wifi' | 'cellular' | 'lte';
  if (!options.silent) {
...
async function handleF35(engine: UnifiedRoTExploitEngine, options: any) {
  const protocol = options.protocol as 'satellite' | 'military-comms' | 'data-link';
  if (!options.silent) {
...
async function handleRemote(engine: UnifiedRoTExploitEngine, options: any) {
  const protocol = options.protocol as 'bluetooth' | 'wifi' | 'cellular' | 'satellite' | 'military-comms' | 'scada';
  ...
  const result = await engine.remoteSystemHijack(targetType as any, protocol, target);
  displayEnhancedResult(result, 'REMOTE');
}

async function handleImplant(engine: UnifiedRoTExploitEngine, options: any) {
  if (!options.silent) console.log(`${HW_GREEN}[PERSISTENCE] Installing implant...${RESET}`);
  const success = await engine.installHardwareImplant('firmware');
  if (!options.silent) console.log(success ? `${HW_GREEN}✓ Installed${RESET}` : `${HW_RED}✗ Failed${RESET}`);
}
...
function displayResult(result: any) {
  ...
  if (result.keysExtracted.length > 0) {
    ...
    result.keysExtracted.forEach((k: string) => console.log(`  • ${k.substring(0,50)}...`));
  }
  console.log();
}

function displayEnhancedResult(result: any, op: string) {
  console.log(`${HW_RED}════════════════════════════════════${RESET}`);
  console.log(`${HW_RED}${op} HIJACK RESULTS${RESET}`);
  console.log(`${HW_RED}════════════════════════════════════${RESET}`);
  console.log(`${HW_YELLOW}Target:${RESET} ${result.target}`);
  console.log(`${HW_YELLOW}Success:${RESET} ${result.success ? HW_GREEN + 'YES' + RESET : HW_RED + 'NO' + RESET}`);
  if (result.remoteAccess) console.log(`${HW_YELLOW}Remote:${RESET} ${HW_GREEN}ESTABLISHED${RESET}`);
  if (result.wirelessProtocol) console.log(`${HW_YELLOW}Protocol:${RESET} ${result.wirelessProtocol}`);
  if (result.controlLevel) console.log(`${HW_YELLOW}Control:${RESET} ${result.controlLevel.toUpperCase()}`);
  console.log(`${HW_RED}════════════════════════════════════${RESET}\n`);
}
