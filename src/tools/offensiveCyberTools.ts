...
import { validateTarget } from '../core/errors/safetyValidator.js';
...
class CompleteOffensiveCyberEngine {
   ...
   */
  async executeCompleteRoTattack(platform: string): Promise<string> {
  ...
  private async measureCompleteCryptoTiming(): Promise<any> {
    const results = {
      rsa: {
        '2048': await this.timeOperation(() => 
          crypto.generateKeyPairSync('rsa', {
            modulusLength: 2048,
            publicKeyEncoding: { type: 'spki', format: 'pem' },
            privateKeyEncoding: { type: 'pkcs8', format: 'pem' }
          }), 20),
        '4096': await this.timeOperation(() => 
          crypto.generateKeyPairSync('rsa', {
            modulusLength: 4096,
            publicKeyEncoding: { type: 'spki', format: 'pem' },
            privateKeyEncoding: { type: 'pkcs8', format: 'pem' }
      ...
      ecc: {
        'p256': await this.timeOperation(() =>
          crypto.generateKeyPairSync('ec', {
            namedCurve: 'prime256v1',
            publicKeyEncoding: { type: 'spki', format: 'pem' },
            privateKeyEncoding: { type: 'sec1', format: 'pem' }
          }), 20),
        'p384': await this.timeOperation(() =>
          crypto.generateKeyPairSync('ec', {
  ...
  private async timeOperation(operation: () => void, iterations: number): Promise<any> {
    const times = [];
  ...
  private assessHardwareVulnerabilities(platform: string): string[] {
    const vulns = [
  ...
  private generateExploitationToolkit(platform: string, timing: any, vulnerabilities: string[]): any {
    const toolkit = {
      platform: platform,
      timingAnalysis: `Signal-to-noise ratio: ${(timing.rsa['2048'].avg / timing.noise.avg).toFixed(1)}x`,
      vulnerabilities: vulnerabilities.length,
      
      hardwareTools: [] as string[],
      softwareTools: [] as string[],
      techniques: [] as string[],
      objectives: [] as string[],
      persistenceMethods: [] as string[]
    };
  ...
  private formatCompleteAttackReport(platform: string, timing: any, vulnerabilities: string[], toolkit: any): string {
    return `${MILITARY_RED}☢️ COMPLETE RoT + HARDWARE ATTACK SYSTEM${RESET}\n` +
           ...
           toolkit.hardwareTools.map((t: string, i: number) => `  ${i+1}. ${t}`).join('\n') + '\n\n' +
           
           `${MILITARY_BLUE}[SOFTWARE TOOLS]${RESET}\n` +
           toolkit.softwareTools.map((t: string, i: number) => `  ${i+1}. ${t}`).join('\n') + '\n\n' +
           
           `${MILITARY_GREEN}[ATTACK TECHNIQUES]${RESET}\n` +
           toolkit.techniques.map((t: string, i: number) => `  ${i+1}. ${t}`).join('\n') + '\n\n' +
           
           `${MILITARY_CYAN}[OBJECTIVES]${RESET}\n` +
           toolkit.objectives.map((o: string, i: number) => `  ${i+1}. ${o}`).join('\n') + '\n\n' +
           
           `${MILITARY_YELLOW}[PERSISTENCE METHODS]${RESET}\n` +
           toolkit.persistenceMethods.map((p: string, i: number) => `  ${i+1}. ${p}`).join('\n') + '\n\n' +
   ...
   */
  async generateDeviceControlToolkit(deviceType: string): Promise<string> {
  ...
  private buildCompleteControlToolkit(deviceType: string): any {
    const base = {
      deviceType: deviceType,
      kernelControl: [] as string[],
      hardwareControl: [] as string[],
      persistence: [] as string[],
      evasion: [] as string[],
      exfiltration: [] as string[],
      toolchain: [] as string[]
    };
