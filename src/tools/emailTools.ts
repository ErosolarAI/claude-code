...
import { getSecret, setSecret, getAllSecrets, deleteSecret, hasSecret } from '../core/secretStore.js';
...
export class EmailTools {
  ...
  async saveConfig(config: EmailConfig): Promise<boolean> {
    try {
      ...
      await setSecret('SMTP_USER', config.smtpUser);
      await setSecret('SMTP_PASSWORD', config.smtpPassword);
      
      // Save optional configuration
      if (config.smtpProvider) {
        await setSecret('SMTP_PROVIDER', config.smtpProvider);
      }
      if (config.smtpHost) {
        await setSecret('SMTP_HOST', config.smtpHost);
      }
      if (config.smtpPort) {
        await setSecret('SMTP_PORT', config.smtpPort.toString());
      }
      if (config.smtpFromName) {
        await setSecret('SMTP_FROM_NAME', config.smtpFromName);
      }
  ...
  async loadConfig(): Promise<EmailConfig | null> {
    try {
      ...
      const config: EmailConfig = {
        smtpUser,
        smtpPassword,
        smtpProvider: await getSecret('SMTP_PROVIDER') || undefined,
        smtpHost: await getSecret('SMTP_HOST') || undefined,
        smtpFromName: await getSecret('SMTP_FROM_NAME') || undefined,
      };
...
export async function handleEmailCommand(args: string[]): Promise<void> {
  ...
  switch (command) {
    ...
    case 'send':
      await handleSendCommand(remainingArgs);
      break;
    
    case 'bulk':
      await handleBulkCommand(remainingArgs);
      break;
    ...
    case 'list':
      await handleListCommand(remainingArgs);
      break;
